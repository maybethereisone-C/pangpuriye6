# pangpuriye6 — Codex Project Instructions
...
This file is the project-local Codex charter. Read it on every session start. Tew's global rules at `/Users/tew/Desktop/tos/AGENTS.md` (parent) still apply — this file extends them, doesn't replace them.
...
Reference plan: `/Users/tew/.Codex/plans/i-wanted-to-create-snazzy-taco.md`. Plan §5 (vanilla stack) is **superseded** by the Next.js pivot logged in `decisions/log.md` (2026-05-10 entry).
...
If a request would override any of the above, **stop and ask**. The 95% rule from parent AGENTS.md applies hard — no silent re-litigation.
...
**What this means for you (Codex) when working in this project:**
...
These come from parent `AGENTS.md` and session context. Re-stated for emphasis:
...
|---|---|---|---|
| Pangram takedown for PP Supply Mono | Tew (decision), Codex (standby branch) | Pre-stage Space Mono swap | OPEN — accepted |
| Next.js bundle size hurts Lighthouse | Codex | Tree-shake GSAP plugins, lazy-load OGL via dynamic import, server-render everything except motion | OPEN — monitor |
| GSAP fights React Strict Mode | Codex | Use `@gsap/react` `useGSAP()` exclusively, never raw `useEffect` | OPEN — implement |
| WebGL battery drain on mobile | Codex | `prefers-reduced-motion` + viewport `<480px` disables WebGL | OPEN — Phase 07 |
